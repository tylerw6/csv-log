...
/**
 *This is part of the included cs50.h
 */
...
int main()
{
    ...
    FILE *techs = fopen("techs.txt", "r");
    int i;
    char tech[40][600];
    if (techs == NULL)
    {
    printf("could not find file techs.txt");
    }
    printf("\nTECHS:\n");
        for (i = 1; !feof(techs); i++)
        {
            fgets(tech[i], 40, techs);
            strtok(tech[i], "\n");
            if (feof(techs))
            break;
            if (i % 2 == 1)
            {
                printf("%2d - %-20s", i, tech[i]);
            }
            else if (i % 2 == 0)
            {
                printf(" %2d - %-20s \n", i, tech[i]);
    ...
    do
    {
     ...
     if (strcmp(tech[name], "Other") == 0)
        {
            printf("Enter your name: ");
            char *other = GetString();
            fprintf(file, "%s,", other);
        }
    else
    fprintf(file, "%s,", tech[name]);

    // OR TIME
    printf("How many minutes were you in the OR? ");
    int ortime = GetInt();
    fprintf(file, "%i,", ortime);
    // OR DOCTOR
    ...
    if (docs == NULL)
    {
        printf("could not find file docs.txt");
    }
    printf("\nDOCTORS:\n");
    for (j = 1; !feof(docs); j++)
    {
    ...
    j--;
    printf( "\nWhat number is the doc? ");

    int docname;
    do
    {
        docname = GetInt();
        if (1 > docname || docname > j)
        printf("Please enter a valid number: ");

    } while (1 > docname || docname > j);

    if (strcmp(doc[docname], "Other") == 0)
        {
            printf("Enter Doctors name: ");
            char *otherdoc = GetString();
            fprintf(file, "%s,", otherdoc);
        }
    else
    fprintf(file, "%s,", doc[docname]);
    // TIME

    time_t nowtime;
    time(&nowtime);
    struct tm *mytime = localtime(&nowtime);
    fprintf (file, "%i/%i/%i %i:%i,", mytime->tm_mon + 1, mytime->tm_mday, mytime->tm_year + 1900, mytime->tm_hour, mytime->tm_min);
    fprintf(file, "\n");
    printf("\n\nThank you, %s, Dr. %s appreciates your %d minutes.\n\n\n", tech[name], doc[docname], ortime);
    //CLEANUP
    fclose(techs);
    fclose(docs);
    fclose(file);
